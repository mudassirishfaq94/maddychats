...
export async function issuePasswordReset(user: UserRow): Promise<boolean> {
  ...
  const html = `<!doctype html><html><body style="margin:0;background:#f4f4ff;font-family:Arial,sans-serif;color:#19182f"><div style="padding:40px 16px"><div style="max-width:560px;margin:auto;background:#fff;border:1px solid #e3e2f4;border-radius:24px;overflow:hidden"><div style="background:#1a1935;color:#fff;padding:28px 32px"><div style="font-size:22px;font-weight:800;letter-spacing:-.5px">Circlo</div><div style="margin-top:6px;color:#c8c4ee;font-size:13px">Your people, your conversations.</div></div><div style="padding:34px 32px"><h1 style="font-size:25px;margin:0 0 14px">Reset your password</h1><p style="font-size:15px;line-height:1.65;color:#52525b">Hi ${name}, we received a request to reset your Circlo password.</p><a href="${resetUrl}" style="display:inline-block;margin:18px 0 22px;background:#6255d8;color:#fff;text-decoration:none;font-weight:700;padding:14px 22px;border-radius:12px">Choose a new password</a><p style="font-size:13px;line-height:1.6;color:#71717a">This secure link expires in one hour and can only be used once. If you didn’t request this, you can safely ignore this email—your password will remain unchanged.</p><div style="margin-top:26px;padding-top:20px;border-top:1px solid #e4e4e7;font-size:12px;color:#a1a1aa">Circlo · App by Mudassir Ishfaq</div></div></div></div></body></html>`;
  const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${process.env.RESEND_API_KEY}`, "Content-Type": "application/json", "Idempotency-Key": `password-reset-${rows[0].id}` }, body: JSON.stringify({ from: process.env.EMAIL_FROM, to: [user.email], subject: "Reset your Circlo password", html, text: `Hi ${user.displayName},\n\nReset your Circlo password using this secure link:\n${resetUrl}\n\nThis link expires in one hour and can only be used once. If you didn't request this, ignore this email.` }) });
  if (!response.ok) {
